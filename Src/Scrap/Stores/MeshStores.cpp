...
namespace Project001
{
    ...
    bool MeshStores::Generate2DLine_v2(
        unsigned int& index,
        const std::vector<glm::vec2>& positions,
        float width,
        size_t endStyle,
    ...
    {
        MeshData newMeshData;
        if (Generate2DLine_v2(newMeshData, meshVertexArray_, meshIndexArray_, positions, width, endStyle, cornerStyle, beveledCorners, triangulate))
        {
            meshDataArray_.push_back(newMeshData);
            index = (unsigned int)(meshDataArray_.size() - 1);
        ...
        bool triangulate)
    {
        ...
        // |/    |/    |/    |/    |/    |/    |
        // +----(B)---(D)----+-----+-----+-----+
        unsigned int indexA = meshData.vertexCount;
        unsigned int indexB;
        unsigned int indexC;
        unsigned int indexD;
        ...
        if (endStyle > 0)
        {
            ...
            indexB = indexA + endStyle;

            for (size_t i = indexA + 2; i <= indexB; ++i)
            {
        ...
        if (endStyle > 0)
        {
            ...
            indexD = indexC + endStyle;

            for (size_t i = indexC + 2; i <= indexD; ++i)
            {
