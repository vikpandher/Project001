...
namespace Project001
{
    ...
    bool TextureStores::LoadTexture(unsigned int& index, const std::string& path)
    {
        ...
        if (!LoadTexture(newTextureData, path))
        {
            return false;
        }
    ...
    bool TextureStores::LoadTexture(TextureData& textureData, const std::string& path)
    {
        textureData.data = stbi_load(path.c_str(), &textureData.width, &textureData.height, &textureData.numberOfComponents, 0);
